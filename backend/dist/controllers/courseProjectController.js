...
export const getCourseProjetById = async (req, res, next) => {
    try {
        ...
        if (!courseProject)
            throw ApiError.notFound("Course project not found");
        const response = {
            success: true,
            data: courseProject,
        };
...
export const getCourseProjectByUserId = async (req, res, next) => {
    ...
    try {
        ...
        const response = { success: true, data: courseProject };
        return res.status(200).json(response);
    }
    catch (err) {
        next(err);
